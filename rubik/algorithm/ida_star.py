...
def ida_star(
    ...
    weight - weight factor to multiply h_cost.
    """
    bound: float = h_cost(start, goal)
    path: dict[State, None] = {start: None}

    while True:
        val: float = search(goal, path, bound, 0.0, h_cost, weight)
...
def search(
    ...
    f_cost = g_cost + weight * h_cost(current, goal)
    ...
    for state in current.successors():
        if state not in path:
            path[state] = None
            val = search(goal, path, bound, g_cost + 1.0, h_cost, weight)
            if val == 0.0:
                return 0.0
            if val < min_val:
                min_val = val
